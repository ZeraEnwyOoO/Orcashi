// orca_crypto.c - Full implementation of ORCA Crypto
...
int orca_aes_cbc_encrypt(const unsigned char* data, size_t data_len,
    ...
    // Allocate output buffer (need extra block for padding)
...
int orca_aes_gcm_encrypt(const unsigned char* data, size_t data_len,
                         ...
                         unsigned char** ciphertext_out,
                         size_t* ciphertext_len) {
    if (!data || !key || !nonce || !tag_out || !ciphertext_out || !ciphertext_len) {
        set_error("NULL pointer in orca_aes_gcm_encrypt");
        return -1;
    }
    ...
    if (RAND_bytes(nonce, 12) != 1) {
        set_error("Failed to generate nonce");
        return -1;
    }
    ...
    *ciphertext_out = (unsigned char*)malloc(data_len + EVP_MAX_BLOCK_LENGTH);
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, 16, tag_out) <= 0) {
        EVP_CIPHER_CTX_free(ctx);
        free(*ciphertext_out);
        *ciphertext_out = NULL;
        set_error("Failed to get GCM tag");
        return -1;
    }
...
int orca_aes_gcm_decrypt(const unsigned char* ciphertext, size_t ciphertext_len,
    ...
    if (!ciphertext || !key || !nonce || !tag || !plaintext_out || !plaintext_len) {
        set_error("NULL pointer in orca_aes_gcm_decrypt");
        return -1;
    }
    ...
    *plaintext_out = (unsigned char*)malloc(ciphertext_len + 1);
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, 16, (void*)tag) <= 0) {
        ...
        free(*plaintext_out);
        *plaintext_out = NULL;
        set_error("Failed to set GCM tag");
        return -1;
    }
    ...
    if (ret <= 0) {
        ...
        free(*plaintext_out);
        *plaintext_out = NULL;
        set_error("Authentication failed - invalid tag");
        return -1;
    }
    total_len += len;
...
int orca_aes_gcm_encrypt_string(const char* str, const char* key_hex,
    ...
    if (!str || !key_hex || !nonce_hex_out || !tag_hex_out || !ciphertext_b64_out) {
        set_error("NULL pointer in orca_aes_gcm_encrypt_string");
        return -1;
    }
    ...
    if (orca_aes_gcm_encrypt((const unsigned char*)str, strlen(str),
                             key, nonce, tag, &ciphertext, &ciphertext_len) < 0) {
        return -1;
    }
    
    orca_bytes_to_hex(nonce, 12, nonce_hex_out);
    orca_bytes_to_hex(tag, 16, tag_hex_out);
...
int orca_aes_gcm_decrypt_string(const char* ciphertext_b64, const char* nonce_hex,
    ...
    if (!ciphertext_b64 || !nonce_hex || !tag_hex || !key_hex || !plaintext_out) {
        set_error("NULL pointer in orca_aes_gcm_decrypt_string");
        return -1;
    }
    ...
    if (orca_hex_to_bytes(nonce_hex, nonce, 12) < 0) {
        set_error("Invalid nonce hex");
        return -1;
    }
    
    unsigned char tag[16];
    if (orca_hex_to_bytes(tag_hex, tag, 16) < 0) {
        set_error("Invalid tag hex");
        return -1;
    }
