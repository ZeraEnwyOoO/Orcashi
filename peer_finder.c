 // peer_cache.c - Peer Cache Implementation in C
...
PeerCache* peer_cache_create(void) {
    ...
    strcpy(pc->cache_file, PEER_CACHE_FILE);
    pc->peer_count = 0;
    pc->modified = false;
...
bool peer_cache_save_peer(PeerCache* pc, const CachePeer* peer) {
    ...
    // Add new peer
    if (pc->peer_count < MAX_CACHE_PEERS) {
...
bool peer_cache_get_peer(PeerCache* pc, const char* id, CachePeer* out_peer) {
    if (!pc || !id || !out_peer) return false;
...
void peer_cache_load(PeerCache* pc) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (in_peer && strstr(line, "\"endpoint\":\"") != NULL) {
            char* start = strstr(line, "\"endpoint\":\"") + 11;
            char* end = strchr(start, '"');
            if (end) {
                int len = end - start;
                strncpy(peer.endpoint, start, len);
                peer.endpoint[len] = '\0';
            }
        }
